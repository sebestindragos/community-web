...
gulp.task('server:clean', () => {
  return gulp.src([
    ...
    'build/main*.{js,map}',
    'build/server',
  ], {read: false, allowEmpty: true}).pipe(rm());
});
...
gulp.task('client:bundle-css', (callback) => {
  return gulp.src('pre-build/client/styles/global.css')
    .pipe(cleanCSS({}))
    .pipe(rename('community.bundle.min.css'))
    .pipe(gulp.dest('build/client'));
});
...
gulp.task('client:watch', () => {
  gulp.watch(['src/client/**/*.ts'], gulp.series(['client:compile']));
  gulp.watch(['src/client/**/*.pug'], gulp.series(['client:pug']));
  gulp.watch(['src/client/**/*.styl'], gulp.series(['client:styl']));
});
 ...
 * Release tasks
 */
gulp.task('server:release', gulp.series([
  'server:clean',
  'server:tslint',
  'server:compile'
]));
gulp.task('client:release', gulp.series([
  'client:clean',
  'client:tslint',
  'client:prepare-aot',
  'client:styl',
